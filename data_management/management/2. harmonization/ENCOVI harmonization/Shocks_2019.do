 ...
 *--------- How the houselhold coped with the shock
 /* (s15q2c): 2c. Cómo se las arregló su hogar con el choque más reciente?*/
	...
	*-- Rename original variable 
	rename s15q2c__`i'* s15q2c_`i'*
	*-- Label original variable
	label var s15q2c_`i'* "2c. Cómo se las arregló su hogar con el choque más reciente?"
	*-- Standarization of missing values
	replace s15q2c_`i'*=. if s15q2c_`i'*==.a
	*-- Generate variable
	clonevar reaccion_evento_`i'* = s15q2c_`i'*
	*-- Label variable
	label var reaccion_evento`i' "Response to event"
	*-- Label values
	label def reaccion_evento`i' "Venta de ganado" 2 "Venta de terreno" 3 "Venta de propiedad" ///
	...
	label val reaccion_evento`i' reaccion_evento`i'
		}
	}
 *--------- 
 /* (s15q2d): */	
	forval i = 1/21 {
	*-- Rename original variable 
	rename s15q2d`i' s15q2d_`i'
	*-- Label original variable
	label var s15q2d_`i' ""
	*-- Standarization of missing values
	replace s15q2d_`i'=. if s15q2d_`i'==.a
	*-- Generate variable
	clonevar mesemig_`i' = s15q2d_`i'
	*-- Label variable
