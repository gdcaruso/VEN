...
mdesc inlanojub if inlist(recibe_ingresonolab,1,2,3)
	*Ok, coincide con los missing a imputar

///*** VARIABLES FOR MINCER EQUATION ***///

	global xvar	edad_sinmis	edad edad2 agegroup hombre relacion_comp npers_viv miembros estado_civil region_est1 entidad municipio ///
...
* Identifying missing values in potential independent variables for Mincer equation
	*Note: "mdesc" displays the number and proportion of missing values for each variable in varlist.
	mdesc $xvar if inlist(recibe_ingresonolab,1,2,3)
	...
	* Generate dependent variable
	gen log_inlanojub = ln(inlanojub) if inlanojub!=.
...
///*** CHECKING WHICH VARIABLES MAXIMIZE R2 ***///
	...
	** LASSO
		...
		*Selected on April 20
		/*
		global lassovars edad hombre miembros p_agegroup_sinmis1 p_agegroup_sinmis2 p_agegroup_sinmis3 p_relacion_comp_sinmis1 p_relacion_comp_sinmis3 p_relacion_comp_sinmis4 ///
		p_relacion_comp_sinmis5 p_relacion_comp_sinmis7 p_relacion_comp_sinmis8 p_relacion_comp_sinmis9 p_relacion_comp_sinmis10 p_estado_civil_sinmis1 p_estado_civil_sinmis5 ///
		p_region_est1_sinmis1 p_region_est1_sinmis4 p_region_est1_sinmis5 p_region_est1_sinmis7 p_region_est1_sinmis8 p_municipio_sinmis1 p_municipio_sinmis2 p_municipio_sinmis6 ///
		p_municipio_sinmis7 p_municipio_sinmis8 p_municipio_sinmis9 p_municipio_sinmis10 p_municipio_sinmis11 p_municipio_sinmis12 p_municipio_sinmis14 p_municipio_sinmis15 ///
		p_municipio_sinmis16 p_municipio_sinmis17 p_municipio_sinmis18 p_municipio_sinmis20 p_municipio_sinmis22 p_municipio_sinmis23 p_tipo_vivienda_hh_sinmis1 p_tipo_vivienda_hh_sinmis2 ///
		p_tipo_vivienda_hh_sinmis3 p_tipo_vivienda_hh_sinmis4 p_tipo_vivienda_hh_sinmis5 p_propieta_hh_sinmis1 p_auto_hh_sinmis1 p_heladera_hh_sinmis1 p_lavarropas_hh_sinmis2 ///
		p_computadora_hh_sinmis1 p_internet_hh_sinmis1 p_televisor_hh_sinmis1 p_calentador_hh_sinmis1 p_afiliado_segsalud_comp_sinmis1 p_afiliado_segsalud_comp_sinmis3 ///
		p_afiliado_segsalud_comp_sinmis4 p_afiliado_segsalud_comp_sinmis5 p_clap_sinmis2 p_ingsuf_comida_sinmis1 p_comida_trueque_sinmis1
		*/
		
	** Vselect
		* Se puede usar R2adjustado como criterio. Ojo, no se puede poner variable como factor variables 
		* return - rpret list
		// vselect log_inlanojub $xvar1 if inlist(recibe_ingresonolab,1,2,3) & log_inlanojub>0 & log_inlanojub!=., best
...
///*** EQUATION ***///
	...
	*(should not be done with "pondera")
	...
	reg log_inlanojub $lassovars if log_inlanojub>0 & log_inlanojub!=. & inlist(recibe_ingresonolab,1,2,3) 
	// reg log_inlanojub $vselectvars if log_inlanojub>0 & log_inlanojub!=. & inlist(recibe_ingresonolab,1,2,3) 
	...
	*set seed 66778899
	...
	mi impute regress log_inlanojub $lassoregress if log_inlanojub>0 & inlist(recibe_ingresonolab,1,2,3) , add(2) rseed(66778899) force noi 
	mi unregister log_inlanojub

///*** REPLACING MISSINGS BY IMPUTED VALUES ***///

	foreach x of varlist inlanojub {
	gen `x'2=.
	* Genero var. con valores preliminares de los missing a imputar
	replace `x'2= exp(log_`x') if inlist(recibe_ingresonolab,1,2,3) & d`x'_miss2==1  
	...
	* Imputo reemplazando en variable ila_m en las variables imputadas (varias porque hay cuantas replicas como repeticiones tenga el vselect)
	...
	mdesc inlanojub if _mi_m==0 & inlist(recibe_ingresonolab,1,2,3)
	mdesc inlanojub if _mi_m==1 & inlist(recibe_ingresonolab,1,2,3) //cheking we do not have incompleted values in inla after the imputation
