...
g newminwage1 = 165 // Extreme poverty-level wage
label var newminwage1 "Extreme poverty-level wage (165 2011-PPP-$)" 
g newminwage2 = 82.5 //Half of extreme poverty-level wage
label var newminwage2 "50 percent of extreme povert-level wage (82.5 2011-PPP-$)"
...
********************************************************************************
*Calculate new income per capita 
********************************************************************************

drop ipcf_sr //from encovi data preparation 

foreach r in sr lr {
* Ingreso individual total Monetario
egen ii_`r' = rsum(Mwage_`r' inla_ppp), missing

* Ingreso familiar total (antes de renta imputada)
egen itf_sin_ri_`r' = sum(ii_`r')	if  hogarsec==0, by(id)

* Ingreso familiar total - total
egen    itf_`r' = rsum(itf_sin_ri_`r' renta_imp_ppp) 
replace itf_`r' = .		if  itf_sin_ri==.

* Ingreso familiar per capita 
gen ipcf_`r'    = itf_`r' / miembros
}



********************************************************************************
*Create dummy for extreme poor
********************************************************************************

foreach r in sr lr {
gen pobre_extremo_`r' = (ipcf_`r' < 102.65) if ipcf_`r' !=missing(ipcf_`r')
gen pobre_`r' = (ipcf_`r' < 260.27) if ipcf_`r' !=missing(ipcf_`r') 
}

tab pobre_extremo_sr [fw=pondera] //52.88
tab pobre_extremo_lr [fw=pondera] //56.96
tab pobre_sr [fw=pondera] //94.5
tab pobre_lr [fw=pondera] //94.8
...
global outcome  pobre_extremo_sr pobre_extremo_lr
...
lab var pobre_extremo_sr "Extreme Poverty Rate (SR)"
lab var pobre_extremo_lr "Extreme Poverty Rate (LR)"

g group="`var'"

order group $outcome

foreach xx in $outcome {
replace `xx'=`xx'*100
}
...
export excel using "$tables\SIMILATION_poverty.xlsx", sheet(`sigma'_`sc') firstrow(varlabels) cell(A2) sheetreplace   


**Save the final excel file:

putexcel set "$tables\SIMILATION_poverty.xlsx", sheet(`sigma'_`sc') modify
