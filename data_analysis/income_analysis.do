...
// For Exports
global excelName	\Income Analysis ENCOVI 2014-2019.xlsx
...
*----------2.3:
	foreach incs of local incs_2018 {
		...
		cuantiles `incs' [w=pondera] if `incs'>=0, n(10) orden_aux(id parentesco edad) g(per_`incs')
		...
		*-- Deciles
		foreach r of numlist 1/10 {
			sum `incs' [aw=pondera] if year==`year' &  per_`incs'==`r', meanonly
			local value = r(mean)
			local type "Deciles `r'"
			post `pname' (`year') ("`incs'") ("`type'") (`value')		
