...
di in g " 2 --- > Non Labor income imputation for employed population "
...
qui {
	...
	*tempvar active inactive edad2 hombre emp_grande emp_mediana
	...
	/* Log Ingreso no laboral de ocupados*/
	gen log_inla_ocu = log(inla_ocu)
	...
	/* missing reports */
	* Labor income
	mdesc inla_ocu if ocupado==1 & (recibe_inla_ocu==1)
...
============================================================================*/
	...
	foreach ind of varlist `independent' {
	 qui mdesc `ind' if ocupado==1 & (recibe_inla_ocu==1)
	 if "`r(miss)'" != "0" {
...
di in y "`miss_var'"
tab ocupado [w=pondera] if recibe_inla_ocu==1 & inla_ocu>0 & inla_ocu!=. 
tab ocupado if recibe_inla_ocu==1 & inla_ocu>0 & inla_ocu!=. 
...
mi impute mvn `miss_var' = `complete_var' if ocupado==1 & recibe_inla_ocu==1 , replace rseed(3593) 
...
mi describe
mi estimate, saving(miest_nl`year'): regress log_inla_ocu `miss_var' `complete_var' [w=pondera] if recibe_inla_ocu==1 & inla_ocu>0 & inla_ocu!=. 
...
gen inla_ocu_imputed = inla_ocu
replace inla_ocu_imputed = xb_mi if ocupado==1 & (recibe_inla_ocu==1) & (inla_ocu==. | inla_ocu==0)
egen inla_ocu_tot_imputed = sum(inla_ocu_imputed), by(id)
label var inla_ocu_imputed "Ingreso no laboral de ocup. imputado - Tot. individual"
label var inla_ocu_tot_imputed "Ingreso no laboral de ocup. imputado - Tot. familiar"
drop xb_mi _mi_* rama_* mun_* nivel_* log_inla_ocu rela_depen
...
====================================================================*/
tempfile non_lab_ocu
save `non_lab_ocu', replace
*stop
use "${output}\ENCOVI_imputed_`year'.dta", clear
*use "${output}\ENCOVI_imputed_`year'.dta", clear
merge 1:1 ano id miembro using `non_lab_ocu', keepusing (inla_ocu_imputed inla_ocu_tot_imputed)
sort ano id miembro
