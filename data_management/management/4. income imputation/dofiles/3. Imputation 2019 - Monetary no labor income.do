...
use "C:\Users\wb563583\WBG\Christian Camilo Gomez Canon - ENCOVI\Databases ENCOVI 2019\data_management\output\for imputation\ENCOVI_forimputation_2019.dta", clear
...
///*** VARIABLES FOR MINCER EQUATION ***///
	...
	local xvar	edad edad2 agegroup hombre relacion_comp npers_viv miembros estado_civil region_est1 entidad municipio ///
				...
				aporte_pension clap ingsuf_comida comida_trueque pelect_monto pgas_monto ptelefono_monto
...
* Identifying missing values in potential independent variables for Mincer equation
	...
	* Dependent variable
	gen log_ijubi_aux = log(ijubi_aux) if ijubi_aux!=.
...
///*** CHECKING WHICH VARIABLES MAXIMIZE R2 ***///
	...
	* Vselect
		...
		* return - rpret list
		vselect log_ijubi_aux `xvar1' if log_ijubi_aux>0 & jubi_pens==1, backward r2adj
		display r(predlist)
...
///*** EQUATION ***///
	...
	*(should not be done with "pondera")
	...
	reg log_ijubi_aux `xvar1' if log_ijubi_aux>0
	reg log_ijubi_aux `vselectvars' if log_ijubi_aux>0
	*reg log_ila_m `nuestrasvars' if log_ila_m>0 // Da peor
	...
	mi set flong
	set seed 66778899
	mi register imputed log_ijubi_aux
	mi impute regress log_ijubi_aux `vselectvars' if log_ijubi_aux>0 & jubi_pens==1, add(1) rseed(66778899) force noi 
	mi unregister log_ijubi_aux
...
* Retrieving original variables
	foreach x of varlist ijubi_aux {
	gen `x'2=.
	local j=1
	...
	*mi unset
	...
	save "$pathdata\VEN_ijubi_imp1.dta" //, replace
...
capture drop _merge
merge 1:1 interview__key interview__id quest using "$pathdata\VEN_ijubi_imp1.dta"
...
putexcel set "$pathout\VEN_income_imputation_2019_MA.xlsx", sheet("labor_ijubi") modify
